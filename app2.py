...
model_url = "https://dlstoragemisango.blob.core.windows.net/dlmisangobeta/Mask_detection.h5"
response = requests.get(model_url)  # Download the model file from the URL
model_bytes = response.content  # Get the binary content of the model file
#model = tf.keras.models.load_model(io.BytesIO(model_bytes), compile=False)  # Load the model from binary data
